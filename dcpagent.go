...
func (agent *DcpAgent) Close() error {
	// TODO(brett19): actually close the streams...
	err := agent.clientMgr.Close()
	if err != nil {
		// TODO(brett19): do something with this error
	}
...
func (agent *DcpAgent) GetVbucketHighSeqNo(ctx context.Context, vbId uint16) (uint64, uint64, error) {
	agent.lock.Lock()
	defer agent.lock.Unlock()

	endpoint, err := agent.vbRouter.DispatchToVbucket(vbId)
	if err != nil {
		return 0, 0, err
	}

	cli, err := agent.clientMgr.GetClient(ctx, endpoint)
	if err != nil {
		log.Printf("failed to get client: %+v", err)
		return 0, 0, err
	}

	statsResp, err := cli.Stats(ctx, &memdx.StatsRequest{
		GroupName: fmt.Sprintf("vbucket-seqno %d", vbId),
	})
	if err != nil {
	...
	vbSeqNoStatName := fmt.Sprintf("vb_%d:high_seqno", vbId)
	vbUuidStatName := fmt.Sprintf("vb_%d:uuid", vbId)

	highSeqNoStr, ok := statsResp.Values[vbSeqNoStatName]
	if !ok {
		return 0, 0, fmt.Errorf("failed to find high_seqno stat for vb %d", vbId)
	}

	highSeqNo, err := strconv.ParseUint(highSeqNoStr, 10, 64)
	if err != nil {
		return 0, 0, fmt.Errorf("failed to parse high_seqno stat for vb %d: %w", vbId, err)
	}

	vbUuidStr, ok := statsResp.Values[vbUuidStatName]
	if !ok {
		return 0, 0, fmt.Errorf("failed to find uuid stat for vb %d", vbId)
	}

	vbUuid, err := strconv.ParseUint(vbUuidStr, 10, 64)
	if err != nil {
		return 0, 0, fmt.Errorf("failed to parse uuid stat for vb %d: %w", vbId, err)
	}

	return highSeqNo, vbUuid, nil
}
