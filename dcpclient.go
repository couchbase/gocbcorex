...
func NewDcpClient(ctx context.Context, opts *DcpClientOptions) (*DcpClient, error) {
	...
	if opts.Priority != "" {
		...
		if err != nil {
			dcpCli.logger.Debug("failed to set dcp priority", zap.Error(err))
		} else {
			dcpCli.priority = opts.Priority
		}
	}
	...
	if opts.ForceValueCompression {
		...
		if err != nil {
			...
			dcpCli.forceValueCompressionEnabled = true
		}
	}
	...
	if opts.EnableExpiryEvents {
		...
		if err != nil {
			...
			dcpCli.expiryEventsEnabled = true
		}
	}
	...
	if opts.EnableStreamIds {
		...
		if err != nil {
			dcpCli.logger.Debug("failed to enable stream ids", zap.Error(err))
		} else {
	...
	if opts.EnableOso {
		if opts.EnableSeqNoAdvance {
			_, err = dcpCli.dcpControl(ctx, &memdx.DcpControlRequest{
				Key:   "enable_out_of_order_snapshots",
				Value: "true_with_seqno_advanced",
			})
			if err != nil {
				dcpCli.logger.Debug("failed to enable oso with seqno advanced", zap.Error(err))
			} else {
				dcpCli.osoEnabled = true
				dcpCli.seqNoAdvancedEnabled = true
			}
		}

		if !dcpCli.osoEnabled {
			_, err = dcpCli.dcpControl(ctx, &memdx.DcpControlRequest{
				Key:   "enable_out_of_order_snapshots",
				Value: "true",
			})
			if err != nil {
				dcpCli.logger.Debug("failed to enable oso", zap.Error(err))
			} else {
				dcpCli.osoEnabled = true
			}
		}
	...
	if opts.BackfillOrder != "" {
		_, err = dcpCli.dcpControl(ctx, &memdx.DcpControlRequest{
			Key:   "backfill_order",
			Value: opts.BackfillOrder,
		})
		if err != nil {
			dcpCli.logger.Debug("failed to set backfill order", zap.Error(err))
		} else {
			dcpCli.backfillOrder = opts.BackfillOrder
		}
	}

	if opts.EnableChangeStreams {
		_, err = dcpCli.dcpControl(ctx, &memdx.DcpControlRequest{
			Key:   "change_streams",
			Value: "true",
		})
		if err != nil {
			dcpCli.logger.Debug("failed to enable change streams", zap.Error(err))
		} else {
			dcpCli.changeStreamsEnabled = true
		}
	}
...
func (c *DcpClient) Telemetry() MemdClientTelem {
	return &noopClientTelem{}
}
