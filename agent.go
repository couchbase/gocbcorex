...
type Agent struct {
	...
	eclientMgr        KvEndpointClientManager
...
func CreateAgent(ctx context.Context, opts AgentOptions) (*Agent, error) {
	...
	*/
	...
	eclientMgr, err := mconnMgr.NewManager(NewManagerOptions{
	...
	agent.vbuuidConsistency = &VbucketConsistencyComponent{}
	...
	agent.crud = &CrudComponent{
		...
		eclientProvider: agent.eclientMgr,
...
func (agent *Agent) NumVbuckets() int {
	...
	if agent.state.latestConfig == nil {
		// this should not be possible, but if it is then we return 0
		return 0
	}

	if agent.state.latestConfig.VbucketMap == nil {
		// this is possible if the agent is a cluster-level agent and does
		// not have a bucket selected.
		return 0
	}
