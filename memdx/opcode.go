...
const (
	...
	OpCodeSrvClustermapChangeNotification = OpCode(0x01)
	OpCodeSrvAuthenticate                 = OpCode(0x02)
	OpCodeSrvActiveExternalUsers          = OpCode(0x03)
)

// String returns the string representation of the OpCode.
func (c OpCode) String(magic Magic) string {
	if magic == MagicReq || magic == MagicRes ||
		magic == MagicReqExt || magic == MagicResExt {
		switch c {
		case OpCodeGet:
			return "Get"
		case OpCodeSet:
	...
	} else if magic == MagicSrvReq || magic == MagicSrvRes {
		switch c {
