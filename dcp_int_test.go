...
func TestDcpBasic(t *testing.T) {
	...
	mgrConfig := &gocbcorex.DcpClientManagerConfig{
		Clients: map[string]*gocbcorex.DcpClientManagerClientConfig{
			"ep-test": {
	...
	mgr, err := gocbcorex.NewDcpStreamClientManager(mgrConfig, &gocbcorex.DcpStreamClientManagerOptions{
		Logger:          logger,
		Bucket:          "default",
	...
	cli, err := mgr.GetClient(ctx, "ep-test")
	require.NoError(t, err)

	stream, err := cli.OpenStream(ctx, &memdx.DcpStreamReqRequest{
		VbucketID:      0,
		Flags:          0,
		StartSeqNo:     0,
