...
func (cc *CrudComponent) Get(ctx context.Context, opts *GetOptions) (*GetResult, error) {
	...
	return OrchestrateSimpleCrud(
		...
		func(collectionID uint32, endpoint string, vbID uint16, client KvClient) (*GetResult, error) {
			cc.recordOptimisedRoutingMetrics(ctx, endpoint)
			...
			func(collectionID uint32) (any, error) {
				...
				for i := uint32(0); i <= numReplicas.Load(); i++ {
					go func(replicaID uint32) {
						for {
							readResult, err := OrchestrateReplicaRead(ctx, cc.vbs, cc.nmvHandler, cc.eclientProvider, opts.Key, replicaID,
							...
							if readResult != nil {
								cc.recordOptimisedRoutingMetrics(ctx, readResult.Endpoint)
							}
...
func (cc *CrudComponent) recordOptimisedRoutingMetrics(ctx context.Context, endpoint string) {
	if cc.disableMetrics {
		return
	}

	if cc.localKvEp == endpoint {
